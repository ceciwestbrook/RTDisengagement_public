...
fsfdir=/ix1/lhallion/TReW-Brain/Scripts/PPI/PPI_level1_fsf_files/PCC_PPI_DC
...
for dir in `ls -d /ix1/lhallion/TReW-Brain/preprocessed/sub-*/`; do
	...
	# not all files have same # of RTs, so have to set for each run
	  ...
	  sed 's/SUBNUM/'${subject}'/g' design_template_level1_PCC_PPI_DC.fsf > $fsfdir'/sub-'${subject}'_ses-01_fsf-lv1_run-'${run}'_PCC_PPI_DC.fsf'
	  sed -i 's/RUNNUM/'${run}'/g' $fsfdir'/sub-'${subject}'_ses-01_fsf-lv1_run-'${run}'_PCC_PPI_DC.fsf'
	  sed -i 's/RTNUM/'${RTnum}'/g' $fsfdir'/sub-'${subject}'_ses-01_fsf-lv1_run-'${run}'_PCC_PPI_DC.fsf'
