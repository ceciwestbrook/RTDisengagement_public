...
fsfdir=/ix1/lhallion/TReW-Brain/Scripts/PPI/PPI_level2_fsf_files/PCC_PPI_ACC
module load fsl
...
if [[ -d "/ix1/lhallion/TReW-Brain/processed/sub-${subject}/ses-01/level2/PCC_PPI_ACC.gfeat/" ]] ; then
	echo "found old feat dir"
	rm -r /ix1/lhallion/TReW-Brain/processed/sub-${subject}/ses-01/level2/PCC_PPI_ACC*.gfeat
	else echo "no old feat dirs found"
fi

for featfile in `ls "$fsfdir/sub-${subject}_ses-01_fsf-lv2_PCC_PPI_ACC.fsf"` ; do
	feat $featfile
done
