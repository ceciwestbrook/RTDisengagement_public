...
fsfdir=/ix1/lhallion/TReW-Brain/Scripts/PPI/PPI_level2_fsf_files/SARTACCcluster
module load fsl
...
if [[ -d "/ix1/lhallion/TReW-Brain/processed/sub-${subject}/ses-01/level2/SARTACCcluster_PPI.gfeat/" ]] ; then
	echo "found old feat dir"
	rm -r /ix1/lhallion/TReW-Brain/processed/sub-${subject}/ses-01/level2/SARTACCcluster_PPI*.gfeat
	else echo "no old feat dirs found"
fi

for featfile in `ls "$fsfdir/sub-${subject}_ses-01_fsf-lv2_SARTACCcluster_PPI.fsf"` ; do
	feat $featfile
done
