...
if [[ -d "/ix1/lhallion/TReW-Brain/processed/sub-${subject}/ses-01/level2/dyscontrolandeffortmods_censorRT.gfeat/" ]] ; then
	echo "found old feat dir"
	rm -r /ix1/lhallion/TReW-Brain/processed/sub-${subject}/ses-01/level2/dyscontrolandeffortmods_censorRT*.gfeat
	else echo "no old feat dirs found"
fi

for featfile in `ls "$fsfdir/sub-${subject}_ses-01_fsf-lv2_dyscontrolandeffortmods.fsf"` ; do
	feat $featfile
done
